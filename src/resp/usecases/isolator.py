...
def getmodel() -> Model:
    ...
    shearStroke: Stroke = Stroke('black', 1)
    damperStroke: Stroke = Stroke('blue', 1)
    for m, s in zip(masterPoints[1:-1], slavePoints[1:]):
        model.spring(m, s, Size(50, 25), shearStroke)
    ...
    for st, ed in zip(startPoints, endPoints):
        ...
        # 枠線
        centerPoint: Point = Utils.get_center_point(st, ed)
        model.tower(centerPoint.addedPoint(0, -nodeDisY * 0.5), centerPoint.addedPoint(0, nodeDisY * 0.5), nodeDisX * 0.42, 12, Stroke('black', 1, [10, 5]))
    ...
    annotationPoint: Point = startPoints[-1].addedPoint(-80, -150)
    annotationFont: Font = Font('black', 18)
    ...
    model.text(annotationPoint, "任意層間配置された免震部材は", annotationFont)
    model.text(annotationPoint.addedPoint(0, 24), "等価曲げせん断モデル時に", annotationFont)
    model.text(annotationPoint.addedPoint(0, 48), "曲げによる水平方向変位が含まれる", annotationFont)
