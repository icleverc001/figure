...
def getmodel() -> Model:
    ...
    for tmp, m, s in zip(tmpNodePoints, masterPoints[:-1], slavePoints):
        ...
        model.dashpot(pleft, disX * tmpRatio, springStroke)
        model.spring(p1, p2, Size(sprLength, 22), springStroke)
        model.line(p2, pright, springStroke)
        model.line(m, pleft, springStroke)
        model.line(s, pright, springStroke)
