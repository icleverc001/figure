...
def getmodel() -> Model:
    ...
    masterPointsTower2: list[Point] = [Point(nodeDisX, -(i * nodeDisY * 0.75) + masterPointsTower1[branchIndex].Y) for i in range(0, 5)]
    ...
    for p1, p2 in zip(masterPointsTower1[1:], masterPointsTower1[:-1]):
        model.line(p1, p2, frameStroke)
    for p1, p2 in zip(masterPointsTower2[1:], masterPointsTower2[:-1]):
        model.line(p1, p2, frameStroke)
    frameStroke: Stroke = Stroke('black', 2, dasharray=[5, 2])
    model.line(masterPointsTower1[branchIndex], masterPointsTower2[0], frameStroke)
    ...
    nodeOffset: int = 35
    springNodeL: Point = masterPointsTower1[2]
    springNodeR: Point = masterPointsTower2[2]
    damperNodeL: Point = masterPointsTower1[-1]
    damperNodeR: Point = masterPointsTower2[-1]
    springTmpPointL: Point = springNodeL.addedPoint(nodeOffset, 0)
    springTmpPointR: Point = springNodeR.addedPoint(-nodeOffset, 0)
    damperTmpPointL: Point = damperNodeL.addedPoint(nodeOffset, 0)
    damperTmpPointR: Point = damperNodeR.addedPoint(-nodeOffset, 0)
    springPointL: Point = springTmpPointL.addedPoint(0, -abs(springNodeR.Y - springNodeL.Y) * 0.5)
    springPointR: Point = springTmpPointR.addedPoint(0, abs(springNodeR.Y - springNodeL.Y) * 0.5)
    damperPointL: Point = damperTmpPointL.addedPoint(0, -abs(damperNodeR.Y - damperNodeL.Y) * 0.5)
    damperPointR: Point = damperTmpPointR.addedPoint(0, abs(damperNodeR.Y - damperNodeL.Y) * 0.5)
    model.line(springNodeL, springTmpPointL, springStroke)
    model.line(springNodeR, springTmpPointR, springStroke)
    model.line(damperNodeL, damperTmpPointL, damperStroke)
    model.line(damperNodeR, damperTmpPointR, damperStroke)
    model.line(springTmpPointL, springPointL, springStroke)
    model.line(springTmpPointR, springPointR, springStroke)
    model.line(damperTmpPointL, damperPointL, damperStroke)
    model.line(damperTmpPointR, damperPointR, damperStroke)
    model.spring(springPointL, springPointR, Size(40, 22), springStroke)
    model.dashpot(damperPointL, damperPointR, Size(14, 28), damperStroke)
    ...
    connectTextPoint: Point = Utils.get_center_point(masterPointsTower1[branchIndex], masterPointsTower2[0]).addedPoint(-20, -10)
    model.text(connectTextPoint, f'接続層', textFont)
